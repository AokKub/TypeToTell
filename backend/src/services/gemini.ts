...
async function geminiMakeInvalidStory(validStory: string) {
  ...
  const response = await ai.models.generateContent({
    model: "gemini-2.0-flash",
    contents: `can you make some 4 spelling errors from this sentence then return it to me?( I want only the story I don't want any introduction or ending words, just the story itself.): "${validStory}"`,
  });
  console.log(response.text);
  return response.text;
}
